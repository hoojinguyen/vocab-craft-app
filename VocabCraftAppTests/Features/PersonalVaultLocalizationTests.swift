...
struct PersonalVaultLocalizationTests {
    private let requiredVaultKeys: [String: (vi: String, en: String)] = [
        ...
        "app.vault.action.review_words": ("⚡ Ôn luyện (%lld từ)", "⚡ Review (%lld words)"),
    ...
    func testAppStringsVaultAccessors() {
        #expect(AppStrings.Vault.titleText == "Kho Từ")
        #expect(AppStrings.Vault.searchPlaceholderText == "Tìm kiếm từ vựng...")
        ...
        #expect(AppStrings.Vault.actionReviewWords(15) == "⚡ Ôn luyện (15 từ)")
        ...
        #expect(AppStrings.Vault.emptySearchNoResultsText == "Không tìm thấy từ nào phù hợp")
        #expect(AppStrings.Vault.detailDefinitionsTitleText == "Định nghĩa")
        #expect(AppStrings.Vault.detailExamplesTitleText == "Ví dụ thực tế")
        #expect(AppStrings.Vault.detailProgressTitleText == "Tiến độ phản xạ")
        #expect(AppStrings.Vault.detailStreakCount(7) == "Chuỗi đúng 7")
        #expect(AppStrings.Vault.detailPracticedModesText == "Chế độ đã luyện")
    }
}
