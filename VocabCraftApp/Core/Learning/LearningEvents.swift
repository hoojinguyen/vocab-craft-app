...
public struct PracticeAttempt: Codable, Hashable, Sendable, Identifiable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        try Self.validateExerciseConstraints(
            exerciseKind: exerciseKind,
            capability: capability,
            container: container
        )
        ...
        try Self.validateEvaluatorAndScores(
    ...
    private static func validateExerciseConstraints(
        exerciseKind: ExerciseKind,
        capability: Capability?,
    ...
    private static func validateEvaluatorAndScores(
        evaluatorVersion: String,
        outcome: ExerciseOutcome,
        scoreMilli: Int?,
        pronunciationScoreMilli: Int?,
        container: KeyedDecodingContainer<CodingKeys>
    ) throws {
        if evaluatorVersion == "unscored" && outcome != .unscored {
            throw DecodingError.dataCorruptedError(
                forKey: .outcome,
                in: container,
        ...
        if outcome == .unscored && scoreMilli != nil {
            throw DecodingError.dataCorruptedError(
                forKey: .scoreMilli,
                in: container,
        ...
        if let score = scoreMilli, !(0...1000).contains(score) {
            throw DecodingError.dataCorruptedError(
                forKey: .scoreMilli,
                in: container,
        ...
        if let pronScore = pronunciationScoreMilli, !(0...1000).contains(pronScore) {
            throw DecodingError.dataCorruptedError(
                forKey: .pronunciationScoreMilli,
                in: container,
